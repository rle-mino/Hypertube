...
module.exports = (app, passport) => {
	...
	const self = {
		login: (req, res, next) => {
			const {error} = Joi.validate(req.body, schema.loginSchema, { abortEarly: false, stripUnknown: true });
		...
		register:  (req, res) => {
			const {error} = Joi.validate(req.body, schema.registerSchema, { abortEarly: false, stripUnknown: true });
			if (error) return res.send({ status: false, details: error.details });
